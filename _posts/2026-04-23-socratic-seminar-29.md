...
### Bitcoin Core v31 released
- cluster mempool/getmempoolcluster changes some RBF behavior to benefit "smart contract" transactions
- new dbcache default for improved performance -- common suggestion for those doing IBD
- asmap
- privatebroadcast

[Bitcoin Core v31](https://bitcoincore.org/en/releases/31.0/)

### Disk space getting low? Consider running bitcoind with blocksdir
With SSD and RAM prices still high those who run bitcoind with a 1TB disk are likely about to run out of space.  For those who don't want to switch to 2TB drives immediatly there is an option in bitcoind to set blocksdir to another drive which can be slower if needed. The blocks directory is seldomly used compared to the read/write intensive chainstate directory which contains the UTXO set.
At current blockheight 946500 the blocksdir sits at about 780GB across ~11,000 files.  Do keep in mind that if need to rescan the chain on a blocks directory from a slower drive would be significantly slower.

[Bitcoinwiki](https://bitcoinwiki.org/wiki/data-directory#blocks_subdirectory)

### UASF BIP-110 v0.4.1
- Adds Expired state for when the Temporary Rules End
- Fixes a "cache-poisoning" bug

[BIP110/444 Implementation RC3](https://github.com/dathonohm/bitcoin/releases/tag/v29.2.knots20251110%2Bbip110-v0.1rc2)
[cache poisoning bug](https://github.com/bitcoinknots/bitcoin/pull/238#issuecomment-3896624715)

### Bitcoin Knots merges BIP-110

[pull request](https://github.com/bitcoinknots/bitcoin/pull/238/)

### BIP-361 and Quantum Nonsense

### Braiins makes available hashrate market
The Hashpower Market lets you buy Bitcoin SHA-256 computing power directly from miners in real time. Prices are transparent, and every order connects you to live supply without hidden costs. It's a simple way to gain exposure to mining without running hardware yourself.
About 30% of Braiins pool hashrate can be purchased from this marketplace

How this is different from cloud mining schemes/scams of the past:
- Purchases can be validated independantly using DATUM or SV2(?) and still mine in a pool
- FPPS or Get paid for hashrate?
